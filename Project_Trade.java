...
class SDisplayt extends JDialog implements ActionListener {
    Connection con;
    ...
    String[] colhed = new String[] {"Trade ID", "Trade Name"};
    Object[][] arr = null;

    SDisplayt(JFrame t, Connection con, String str, boolean state) {
        super(t, str, true);
        this.con = con;
        ...
        b1 = new JButton("Trade");
        b2 = new JButton("Sort by Trade Name");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (b == b4) {
            ...
            if (b == b1) {
                ...
                query = "SELECT * FROM trade ORDER BY tradename";
            }
            ...
            try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    dtm.addRow(new Object[]{rs.getInt("tradeid"), rs.getString("tradename")});
                }
...
public class Project_Trade extends JFrame implements ActionListener, FocusListener {
    Connection con;
    JPanel p1, p2;
    JLabel l3, l4;
    JTextField t3, t4;
    ...
    Project_Trade(Connection con) {
        super("Trade Management");
        this.con = con;
        ...
        b4.setForeground(Color.WHITE); //#f9e8c3
        // p2 = new JPanel();
        p1.add(b1);
        p1.add(b2);
        p1.add(b3);
    ...
    public void focusLost(FocusEvent e) {
        ...
        if (t == t3) {
            try {
                ...
                PreparedStatement ps = con.prepareStatement("SELECT * FROM trade WHERE tradeid=?");
                ps.setInt(1, no);
                rs = ps.executeQuery();

                if (rs.next()) {
                    tnm = rs.getString("tradename");
                    t4.setText(tnm);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (b == b1) {
            ...
            new SDisplayt(this, con, "Trade Display", true);
        }
        t3.setText("");
        t4.setText("");
        t3.requestFocus();
    ...
    void addRecord() {
        try {
            PreparedStatement ps = con.prepareStatement("INSERT INTO trade (tradeid, tradename) VALUES (?, ?)");
    ...
    void modifyRecord() {
        try {
            PreparedStatement ps = con.prepareStatement("UPDATE trade SET tradename=? WHERE tradeid=?");
    ...
    void deleteRecord() {
        try {
            PreparedStatement ps = con.prepareStatement("DELETE FROM trade WHERE tradeid=?");
            ps.setInt(1, Integer.parseInt(t3.getText()));
            ps.executeUpdate();
        } catch (SQLException e) {
